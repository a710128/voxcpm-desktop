...
impl CudaDevice {
    ...
    pub fn clone_htod<T: cudarc::driver::DeviceRepr, Src: cudarc::driver::HostSlice<T> + ?Sized>(
        ...
        // During CUDA graph capture, HtoD copies from pageable host memory often force an
        // implicit stream synchronize to keep the host pointer alive. That breaks capture.
        //
        // When a capture is active, we stage into pinned host memory and keep it alive in
        // the capture keepalive store until the captured graph is dropped.
        if let Some(cap) = super::active_capture() {
            cap.validate(self)?;
            let (src_slice, _sync) = unsafe { src.stream_synced_slice(&self.stream) };
            let mut alloc = super::graph_capture::GraphPinnedHostAlloc::alloc(
                &self.context,
                src_slice.len() * std::mem::size_of::<T>(),
            )?;
            alloc.copy_from_bytes(
                src_slice.as_ptr() as *const u8,
                src_slice.len() * std::mem::size_of::<T>(),
            );
            let view = alloc.view::<T>(src_slice.len());
            let out = self.stream.clone_htod(&view).w()?;
            cap.push_keepalive(Box::new(alloc));
            Ok(out)
        } else {
